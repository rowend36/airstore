...
function prepare(stmt) {
    if (cache.has(stmt)) return cache.get(stmt);
        ...
        console.log(stmt);
        console.log(
            db.prepare("EXPLAIN QUERY PLAN " + stmt).all(
                ...stmt
                    .split("?")
                    .slice(1)
                    .map((e) => "")
    ...
    */
    ...
    _select(query, count) {
        ...
        return db.transaction(() => {
            ...
            if (count) {
                compiled =
                    "SELECT count(*) as ___count__ FROM(" + compiled + ")";
            }
    ...
    set(data, conditions = {}) {
        ...
        db.transaction(() => {
            ...
            if (!res.changes) {
                ...
                if (
                    !(
                        create &&
                        this.remove(data) &&
                        (v = prepare(create.sql).run(create.args)).changes > 0
                    )
                ) {
            ...
            prepare(`INSERT INTO ${this.oplog} (__id__) VALUES (?)`).run(
                flat.__id__
            );
        })();
    }
    ...
    remove(data) {
        if (!data.__id__) throw new Error("No id provided");
        return (
            prepare(`DELETE FROM ${this.name} WHERE __id__ == ?`).run(
                data.__id__
            ).changes > 0
        );
    }
    ...
    createColumns(cols) {
        ...
        for (let col of cols) {
            ...
            `
                    ...
                    db.prepare(
                        `CREATE INDEX IF NOT EXISTS ${clean(
                            prefix + "idx___"
                        )} ON ${this.name} (json_extract(${clean(col)}, '$'))`
                    ).run();
                })();
                ...
                if (
                    e.code === "SQLITE_ERROR" &&
                    /duplicate column/.test(e.message)
                ) {
                    this.knownColumns.push(clean(col));
                    continue;
                }
