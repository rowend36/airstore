...
function createValidator(str) {
    ...
    function checkEndScopes(expression, isFunction, leaveInnerScopes) {
        ...
        while (peek() === "in match" || peek() === "in service") {
            ...
            if (pop() /*match|service*/ === "match")
                matchers.push(matchRe.pop());
    ...
    try {
        for (let line of lines) {
            ...
            //match a line
                ...
                startScope();
                matchRe.push(
                    createMatchRe(match[1], scopes[scopes.length - 1])
                );
                ...
                if (
                    (bad = methods.find(
                        (e) => !matchRe[matchRe.length - 1].allows[e]
                    ))
                )
                ...
                closeExpression();
                const parsed = /^\s*function\b\s*([a-z_][a-z_0-9]*).*$/.exec(
                    line
                );
...
module.exports = createValidator(require("./firestore-rules"));