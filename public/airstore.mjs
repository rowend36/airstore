...
var require_props = __commonJS({
  "src/database/props.js"(exports) {
    ...
    exports.clean = function clean2(key) {
      return inbuiltProps2[key] || key.startsWith('"___') ? key : JSON.stringify(
...
function ref(path, airstore) {
  if (/___/.test(path) || !/^[a-z][a-z_0-9]+(\/[a-z][a-z_0-9]+\/[a-z][a-z_0-9]+)+/.test(path))
...
function collection(...path) {
  let airstore = typeof path[0] == "function" ? path.shift() : null;
  const m = path.join("/");
...
function doc(...path) {
  ...
  if (typeof path[0] == "object") {
    if (path[0].api) {
      ...
      path[0] = path[0].path;
      if (path.length === 1)
        path.push(v4_default());
    }
  }
  const m = path.join("/") ?? v4_default();
  ...
  for (let spec of specs) {
    if (spec.cursor) {
      for (let i = 0; i < spec.cursor.length; i++) {
        specs.push({
          filter: {
            prop: typeof spec.cursor[i] === "object" ? documentId() : spec.order.props[i],
            op: spec.op,
            val: spec.cursor[i]
          }
        });
...
function toSnapshot(_query, prevSnapshot, res, isDoc) {
  const docs = res.rows.map((e) => {
    if (typeof e === "string") {
      return prevSnapshot.docs.find((e2) => e2.id());
    } else
