...
function validCollection(path) {
  return !/___/.test(path) && /^[a-z][a-z_0-9]+(\/[a-z][a-z_0-9]+\/[a-z][a-z_0-9]+)*$/.test(path);
}
function clean(key, m) {
  if (m !== true)
    console.log(key, clean(key, true));
  return inbuiltProps[key] || key.startsWith('"___') ? key : /(?:^[^a-z_]|[^a-z0-9_])/.test(key) ? JSON.stringify(
    key.startsWith("___") ? key : "___" + key.replace(/\./g, "___") + "__"
  ) : key.startsWith("___") ? key : "___" + key.replace(/\./g, "___") + "__";
}
function unflatten(row, columnNames) {
  const s = {};
  for (let i = 0; i < columnNames.length; i++) {
    if (inbuiltProps[columnNames[i]])
      return;
    let key = columnNames[i];
    if (key.slice(0, 3) === "___") {
      let i2 = key.slice(3, -2);
      if (i2.indexOf("___") > -1) {
        const path = i2.split("___");
        let ctx = obj;
        for (let j = 0; j < path.length - 1; j++) {
          ctx = ctx[path[j]] = {};
        }
        ctx[path[path.length - 1]] = row[i2];
      } else {
        obj[i2] = row[i2];
      }
    } else
...
var DocumentSnapshot = class {
  ...
  data(options) {
    ...
    if (!this._data)
      return;
    return unflatten(this._data, this.columns);
  }
  exists() {
    return !!this._data;
  }
  get(prop) {
    return this._data[this.columns.indexOf(clean(prop))];
  }
  version() {
